...
const actions = {
  ...
  getItem ({ commit }, id) {
    ...
    return axios
    .post(constants.baseUrl + 'pos/customer/' + id)
      .then(response => {
  ...
  getItems ({ commit, state }, qurey) {
    ...
    if(!state.loading) {
      commit('setLoading', true)
      axios
        .post(constants.baseUrl + 'pos/customer/list',qurey)
        .then(response => {
          commit('setItems', response.data)
          let ids = []
          for(let item of response.data.rows){
              if(ids.indexOf(item.customer)<0){
                  ids.push(item.customer);
              }
  ...
  createItem ({ commit }, payload) {
    ...
    console.log(payload)
    return axios
      .post(constants.baseUrl + 'pos/customer',payload)
      .then(() => {
  ...
  updateItem ({ commit }, payload) {
    ...
    console.log(payload)
    return axios
      .put(constants.baseUrl + 'pos/customer/' + payload._id,payload)
      .then(() => {
  ...
  deleteItem ({ commit }, id) {
    ...
    return axios
      .delete(constants.baseUrl + 'pos/customer/' + id)
      .then(() => {
