      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">Permissions</span>
...
export default {
  ...
  computed: {
    ...
    success() {
      return this.$store.state.permission.success;
    },
    error() {
      return this.$store.state.permission.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.permission.items &&
        this.$store.state.permission.items.rows
      ) {
        this.$store.state.permission.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.permission.items.total;
    },
    limit() {
      return this.$store.state.permission.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.permission.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("permission/clearState");
    this.getItems();
  },
  methods: {
    next() {
    ...
    async fakeItems() {
      for (var i = 0; i < 100; i++) {
        ...
        await this.$store.dispatch("permission/createItem", payload);
      }
    },
    add() {
      this.editedItem = {};
    ...
    async getItems() {
      ...
      await this.$store.dispatch("permission/getItems", data);
    },
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("permission/updateItem", {
        ...
        if (
          await this.$store.dispatch("permission/createItem", {
            name: this.editedItem.name,
            userGroup: this.editedItem.userGroup._id,
            user: this.editedItem.user._id
          })
