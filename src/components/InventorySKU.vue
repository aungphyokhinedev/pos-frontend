      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">Inventroy SKUs</span>
        ...
        <template v-slot:item.skuCode="{ item }">
          <a @click="edit(item)">{{item.skuCode ? item.skuCode: '-'}}</a>
        </template>
...
export default {
  computed: {
    loading() {
      return this.$store.state.inventorysku.loading;
    },
    success() {
      return this.$store.state.inventorysku.success;
    },
    error() {
      return this.$store.state.inventorysku.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.inventorysku.items &&
        this.$store.state.inventorysku.items.rows
      ) {
        this.$store.state.inventorysku.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.inventorysku.items.total;
    },
    limit() {
      return this.$store.state.inventorysku.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.inventorysku.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("inventorysku/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
    ...
    async getItems() {
      ...
      await this.$store.dispatch("inventorysku/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("inventorysku/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("inventorysku/updateItem", {
            name: this.editedItem.name,
            skuCode: this.editedItem.skuCode,
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("inventorysku/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
