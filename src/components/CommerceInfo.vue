      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">Commerce Infos</span>
...
export default {
  ...
  computed: {
    loading() {
      return this.$store.state.commerceinfo.loading;
    },
    success() {
      return this.$store.state.commerceinfo.success;
    },
    error() {
      return this.$store.state.commerceinfo.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.commerceinfo.items &&
        this.$store.state.commerceinfo.items.rows
      ) {
        this.$store.state.commerceinfo.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.commerceinfo.items.total;
    },
    limit() {
      return this.$store.state.commerceinfo.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.commerceinfo.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("commerceinfo/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
    ...
    async getItems() {
      ...
      await this.$store.dispatch("commerceinfo/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("commerceinfo/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("commerceinfo/updateItem", {
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("commerceinfo/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
