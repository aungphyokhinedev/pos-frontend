      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">Suppliers' Items</span>
      ...
      <v-card v-if="isadding">
        ...
        <v-card-text>
          <v-col cols="12" style="padding:0">
           ...
           <v-text-field
              dense
              v-model="editedItem.deliveryLeadDays"
              type="number"
              label="Devlivery Lead days"
              append-icon="text_format"
              :counter="50"
            ></v-text-field>
            ...
            <v-text-field
              dense
              v-model="editedItem.standardPrice"
              type="number"
            ...
            ></v-text-field>
             <v-text-field
              dense
              v-model="editedItem.discountedPrice"
              type="number"
...
export default {
  ...
  computed: {
    ...
    loading() {
      return this.$store.state.inventorysupplieritem.loading;
    },
    success() {
      return this.$store.state.inventorysupplieritem.success;
    },
    error() {
      return this.$store.state.inventorysupplieritem.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.inventorysupplieritem.items &&
        this.$store.state.inventorysupplieritem.items.rows
      ) {
        this.$store.state.inventorysupplieritem.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.inventorysupplieritem.items.total;
    },
    limit() {
      return this.$store.state.inventorysupplieritem.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.inventorysupplieritem.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("inventorysupplieritem/clearState");
    this.getItems();
  },
  methods: {
    ...
    async getItems() {
      ...
      await this.$store.dispatch("inventorysupplieritem/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("inventorysupplieritem/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("inventorysupplieritem/updateItem", {
            name: this.editedItem.name,
            supplier: this.editedItem.supplier,
            item: this.editedItem.item,
            totalValue: this.editedItem.totalValue,
            totalQty: this.editedItem.totalQty,
            minQty: this.editedItem.minQty,
            maxQty: this.editedItem.maxQty,
            deliveryLeadDays: this.editedItem.deliveryLeadDays,
            standardPrice: this.editedItem.standardPrice,
            discountedPrice: this.editedItem.discountedPrice,
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("inventorysupplieritem/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
