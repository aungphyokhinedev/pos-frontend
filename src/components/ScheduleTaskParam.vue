      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">Task Params</span>
...
export default {
  ...
  computed: {
    loading() {
      return this.$store.state.scheduletaskparam.loading;
    },
    success() {
      return this.$store.state.scheduletaskparam.success;
    },
    error() {
      return this.$store.state.scheduletaskparam.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.scheduletaskparam.items &&
        this.$store.state.scheduletaskparam.items.rows
      ) {
        this.$store.state.scheduletaskparam.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.scheduletaskparam.items.total;
    },
    limit() {
      return this.$store.state.scheduletaskparam.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.scheduletaskparam.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("scheduletaskparam/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
    ...
    async getItems() {
      ...
      await this.$store.dispatch("scheduletaskparam/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("scheduletaskparam/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("scheduletaskparam/updateItem", {
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("scheduletaskparam/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
