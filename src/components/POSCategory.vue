      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">POS Categories</span>
...
export default {
  ...
  computed: {
    loading() {
      return this.$store.state.poscategory.loading;
    },
    success() {
      return this.$store.state.poscategory.success;
    },
    error() {
      return this.$store.state.poscategory.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.poscategory.items &&
        this.$store.state.poscategory.items.rows
      ) {
        this.$store.state.poscategory.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.poscategory.items.total;
    },
    limit() {
      return this.$store.state.poscategory.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.poscategory.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("poscategory/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
    ...
    async getItems() {
      ...
      if (this.filterChange.query) {
        ...
        data.searchFields = "name,categoryCode";
      }
      ...
      await this.$store.dispatch("poscategory/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("poscategory/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("poscategory/updateItem", {
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("poscategory/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
