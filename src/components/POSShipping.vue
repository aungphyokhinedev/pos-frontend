      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">POS Shippings</span>
...
export default {
  ...
  computed: {
    loading() {
      return this.$store.state.posshipping.loading;
    },
    success() {
      return this.$store.state.posshipping.success;
    },
    error() {
      return this.$store.state.posshipping.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.posshipping.items &&
        this.$store.state.posshipping.items.rows
      ) {
        this.$store.state.posshipping.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.posshipping.items.total;
    },
    limit() {
      return this.$store.state.posshipping.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.posshipping.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("posshipping/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
    ...
    async getItems() {
      ...
      await this.$store.dispatch("posshipping/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("posshipping/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("posshipping/updateItem", {
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("posshipping/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
