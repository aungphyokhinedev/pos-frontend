      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">Services</span>
...
export default {
  computed: {
    ...
    success() {
      return this.$store.state.service.success;
    },
    error() {
      return this.$store.state.service.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.service.items &&
        this.$store.state.service.items.rows
      ) {
        this.$store.state.service.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.service.items.total;
    },
    limit() {
      return this.$store.state.service.items.limit;
    },
    pageChange() {
      return (
  ...
  created() {
    this.$store.dispatch("service/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
    ...
    async getItems() {
      ...
      await this.$store.dispatch("service/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("service/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("service/updateItem", {
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("service/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
