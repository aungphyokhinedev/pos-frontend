...
{{error}}
<v-icon class="mx-2" color="error" small @click="clear()">
  close
</v-icon>
    </v-row>
          ...
          >
            <template v-slot:top>
              <v-toolbar dark color="primary" flat>
                ...
                <span class="title mr-2">Applications</span>
...
export default {
  ...
  computed: {
    ...
    items() {
      var result = [];
      if (this.$store.state.application.items && this.$store.state.application.items.rows) {
        this.$store.state.application.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.application.items.total;
    },
    limit() {
      return this.$store.state.application.items.limit;
    },
    pageChange() {
      return this.page + "" + this.pageSize + "" + this.sort + "" + this.sortOrder;
    },
    pagination() {
      return this.$store.state.application.pagination;
    }
  },
  watch: {
    error(){
  ...
  created() {
    this.$store.dispatch("application/clearState");
    this.getItems()
  },
  methods: {
    next() {
    ...
    async getItems() {
      ...
      await this.$store.dispatch("application/getItems",data);
    },

    async deleteUser() {
      
      if(confirm("Are you sure you want to delete this item?")){
        if(await this.$store.dispatch("application/deleteItem",this.editedItem._id )){
          this.getItems();
          this.close();
        }
      }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if(await this.$store
          .dispatch("application/updateItem", {
        ...
        console.log("add new");
        if(await this.$store
          .dispatch("application/createItem", this.editedItem)){
            this.getItems();
            this.close();
          }
      }
