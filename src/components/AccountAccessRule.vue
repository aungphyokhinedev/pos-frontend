      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">Account Access Rules</span>
        ...
        <template v-slot:item.canDebit="{ item }">
          <a>{{item.canDebit}}</a>
        </template>
        <template v-slot:item.canCredit="{ item }">
          <a>{{item.canCredit}}</a>
        </template>
          ...
          <v-col cols="12" style="padding:0">
            ...
            ></v-text-field>
             <v-row style="padding:10px" >
              <v-checkbox class="mr-3" dense v-model="editedItem.canDebit" label="Can Debit" required></v-checkbox>
                <v-checkbox class="mr-3" dense v-model="editedItem.canCredit" label="Can Credit" required></v-checkbox>
                 <v-checkbox class="mr-3" dense v-model="editedItem.isPrivate" label="Is Private" required></v-checkbox>
...
export default {
  computed: {
    loading() {
      return this.$store.state.accountaccessrule.loading;
    },
    success() {
      return this.$store.state.accountaccessrule.success;
    },
    error() {
      return this.$store.state.accountaccessrule.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.accountaccessrule.items &&
        this.$store.state.accountaccessrule.items.rows
      ) {
        this.$store.state.accountaccessrule.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.accountaccessrule.items.total;
    },
    limit() {
      return this.$store.state.accountaccessrule.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.accountaccessrule.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("accountaccessrule/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
    ...
    async getItems() {
      ...
      await this.$store.dispatch("accountaccessrule/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("accountaccessrule/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("accountaccessrule/updateItem", {
            name: this.editedItem.name,
            lowerLimit: this.editedItem.lowerLimit,
            upperLimit: this.editedItem.upperLimit,
            canDebit: this.editedItem.canDebit,
            canCredit: this.editedItem.canCredit,
            isPrivate: this.editedItem.isPrivate,
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("accountaccessrule/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
