...
export default {
  ...
  methods: {
    ...
    changeRoom(value) {
      this.editedItem.room = value._id;
    },
    next() {
      if (!this.isadding) {
        this.add();
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("socketroomuser/updateItem", {
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("socketroomuser/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
