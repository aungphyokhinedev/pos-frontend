      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">Users Groups</span>
...
export default {
  computed: {
    loading() {
      return this.$store.state.usergroup.loading;
    },
    success() {
      return this.$store.state.usergroup.success;
    },
    error() {
      return this.$store.state.usergroup.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.usergroup.items &&
        this.$store.state.usergroup.items.rows
      ) {
        this.$store.state.usergroup.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.usergroup.items.total;
    },
    limit() {
      return this.$store.state.usergroup.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.usergroup.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("usergroup/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
    ...
    async fakeItems() {
      for (var i = 0; i < 100; i++) {
        ...
        await this.$store.dispatch("usergroup/createItem", payload);
      }
    },
    add() {
      this.editedItem = {};
    ...
    async getItems() {
      ...
      await this.$store.dispatch("usergroup/getItems", data);
    },
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("usergroup/updateItem", {
        ...
        console.log("add new");
        if (
          await this.$store.dispatch("usergroup/createItem", this.editedItem)
        ) {
          this.getItems();
          this.close();
        }
