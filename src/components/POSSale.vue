      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">POS Sales</span>
        ...
        <template v-slot:item.invoiceNumber="{ item }">
          <a @click="edit(item)">{{item.invoiceNumber ? item.invoiceNumber: '-'}}</a>
        </template>
        ...
        <v-card-text>
          <v-col cols="12" style="padding:0">
            ...
            ></v-text-field>    
             <v-text-field
              dense
              v-model="editedItem.adjustmentRemark"
              label="Adjusment Remark"
              append-icon="text_format"
              :counter="50"
            ></v-text-field>    
...
export default {
  ...
  computed: {
    loading() {
      return this.$store.state.possale.loading;
    },
    success() {
      return this.$store.state.possale.success;
    },
    error() {
      return this.$store.state.possale.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.possale.items &&
        this.$store.state.possale.items.rows
      ) {
        this.$store.state.possale.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.possale.items.total;
    },
    limit() {
      return this.$store.state.possale.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.possale.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("possale/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
    ...
    async getItems() {
      ...
      await this.$store.dispatch("possale/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("possale/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("possale/updateItem", {
            ...
            adjustmentRemark: this.editedItem.adjustmentRemark,
            total: this.editedItem.total,
            tax: this.editedItem.tax,
            adjustment: this.editedItem.adjustment,
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("possale/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
