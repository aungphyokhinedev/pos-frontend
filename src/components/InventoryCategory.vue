      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">Inventroy Categories</span>
        ...
        <template v-slot:item.categoryCode="{ item }">
          <a @click="edit(item)">{{item.categoryCode ? item.categoryCode: '-'}}</a>
        </template>
...
export default {
  computed: {
    loading() {
      return this.$store.state.inventorycategory.loading;
    },
    success() {
      return this.$store.state.inventorycategory.success;
    },
    error() {
      return this.$store.state.inventorycategory.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.inventorycategory.items &&
        this.$store.state.inventorycategory.items.rows
      ) {
        this.$store.state.inventorycategory.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.inventorycategory.items.total;
    },
    limit() {
      return this.$store.state.inventorycategory.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.inventorycategory.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("inventorycategory/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
    ...
    async getItems() {
      ...
      await this.$store.dispatch("inventorycategory/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("inventorycategory/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("inventorycategory/updateItem", {
            name: this.editedItem.name,
            categoryCode: this.editedItem.categoryCode,
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("inventorycategory/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
