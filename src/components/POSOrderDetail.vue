      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">POS Order Details</span>
...
export default {
  ...
  computed: {
    loading() {
      return this.$store.state.posorderdetail.loading;
    },
    success() {
      return this.$store.state.posorderdetail.success;
    },
    error() {
      return this.$store.state.posorderdetail.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.posorderdetail.items &&
        this.$store.state.posorderdetail.items.rows
      ) {
        this.$store.state.posorderdetail.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.posorderdetail.items.total;
    },
    limit() {
      return this.$store.state.posorderdetail.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.posorderdetail.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("posorderdetail/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
  ...
  changeOrder(value) {
    ...
    async getItems() {
      ...
      await this.$store.dispatch("posorderdetail/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("posorderdetail/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("posorderdetail/updateItem", {
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("posorderdetail/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
