      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">Account Units</span>

            <v-spacer></v-spacer>
            <v-icon small @click="getItems">refresh</v-icon>
          </v-toolbar>
        </template>
        <template v-slot:item.unitCode="{ item }">
          <a @click="edit(item)">{{item.unitCode ? item.unitCode: '-'}}</a>
        </template>
...
export default {
  computed: {
    loading() {
      return this.$store.state.accountunit.loading;
    },
    success() {
      return this.$store.state.accountunit.success;
    },
    error() {
      return this.$store.state.accountunit.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.accountunit.items &&
        this.$store.state.accountunit.items.rows
      ) {
        this.$store.state.accountunit.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.accountunit.items.total;
    },
    limit() {
      return this.$store.state.accountunit.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.accountunit.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("accountunit/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
    ...
    async getItems() {
      ...
      if (this.filterChange.query) {
        ...
        data.searchFields = "name,unitCode";
      }
      ...
      await this.$store.dispatch("accountunit/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("accountunit/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("accountunit/updateItem", {
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("accountunit/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
