      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">Commerce Shippings</span>
...
export default {
  ...
  computed: {
    loading() {
      return this.$store.state.commerceshipping.loading;
    },
    success() {
      return this.$store.state.commerceshipping.success;
    },
    error() {
      return this.$store.state.commerceshipping.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.commerceshipping.items &&
        this.$store.state.commerceshipping.items.rows
      ) {
        this.$store.state.commerceshipping.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.commerceshipping.items.total;
    },
    limit() {
      return this.$store.state.commerceshipping.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.commerceshipping.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("commerceshipping/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
    ...
    async getItems() {
      ...
      await this.$store.dispatch("commerceshipping/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("commerceshipping/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("commerceshipping/updateItem", {
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("commerceshipping/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
