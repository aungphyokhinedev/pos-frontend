      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">Read Write Rules</span>
        ...
        <v-card-text>
          <v-col cols="12" style="padding:0">
            ...
            <v-row style="padding:10px">
              <v-checkbox class="mr-3" dense v-model="editedItem.read" label="Read" required></v-checkbox>
              <v-checkbox class="mr-3" dense v-model="editedItem.create" label="Create" required></v-checkbox>
              <v-checkbox class="mr-3" dense v-model="editedItem.update" label="Update" required></v-checkbox>
              <v-checkbox dense v-model="editedItem.delete" label="Delete" required></v-checkbox>
            </v-row>
          </v-col>
...
export default {
  computed: {
    loading() {
      return this.$store.state.rule.loading;
    },
    success() {
      return this.$store.state.rule.success;
    },
    error() {
      return this.$store.state.rule.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (this.$store.state.rule.items && this.$store.state.rule.items.rows) {
        this.$store.state.rule.items.rows.forEach(item => {
          var newitem = Object.assign({}, item);
          newitem.createdAt = moment(newitem.createdAt).format("D-MMM-YY");
          newitem.updatedAt = moment(newitem.updatedAt).format("D-MMM-YY");
    ...
    total() {
      return this.$store.state.rule.items.total;
    },
    limit() {
      return this.$store.state.rule.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.rule.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("rule/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
    ...
    async fakeItems() {
      for (var i = 0; i < 100; i++) {
        ...
        await this.$store.dispatch("rule/createItem", payload);
      }
    },
     next() {
      if(!this.isadding){
    ...
    async getItems() {
      ...
      await this.$store.dispatch("rule/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("rule/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("rule/updateItem", {
            name: this.editedItem.name,
            _id: this.editedItem._id,
            read: this.editedItem.read,
            create: this.editedItem.create,
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("rule/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
  ...
  data: () => ({
    ...
    headers: [
      ...
      { text: "Read", value: "read", width: 90 },
      { text: "Create", value: "create", width: 90 },
      { text: "Update", value: "update", width: 90 },
      { text: "Delete", value: "delete", width: 90 },

      { text: "Created At", value: "createdAt", width: 110 },

      { text: "Update At", value: "updatedAt", width: 110 }
    ]
  })
};
</script>