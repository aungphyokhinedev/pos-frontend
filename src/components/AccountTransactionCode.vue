      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">Transaction Codes</span>
        ...
        <template v-slot:item.transactionCode="{ item }">
          <a @click="edit(item)">{{item.transactionCode ? item.transactionCode: '-'}}</a>
        </template>
...
export default {
  computed: {
    loading() {
      return this.$store.state.accounttransactioncode.loading;
    },
    success() {
      return this.$store.state.accounttransactioncode.success;
    },
    error() {
      return this.$store.state.accounttransactioncode.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.accounttransactioncode.items &&
        this.$store.state.accounttransactioncode.items.rows
      ) {
        this.$store.state.accounttransactioncode.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.accounttransactioncode.items.total;
    },
    limit() {
      return this.$store.state.accounttransactioncode.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.accounttransactioncode.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("accounttransactioncode/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
    ...
    async getItems() {
      ...
      if (this.filterChange.query) {
        ...
        data.searchFields = "name,transactionCode";
      }
      ...
      await this.$store.dispatch("accounttransactioncode/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("accounttransactioncode/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("accounttransactioncode/updateItem", {
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("accounttransactioncode/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
