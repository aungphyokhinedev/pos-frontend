      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">Inventroy Suppliers</span>
        ...
        <v-card-text>
          <v-col cols="12" style="padding:0">
            ...
            <v-text-field
              ...
              :counter="50"
            ></v-text-field>
            <v-file-input prepend-icon="mdi-camera" @change="changeLogo" v-model="files" show-size label="Logo File"></v-file-input>
            
            <v-text-field
              dense
              v-model="editedItem.typeGoods"
              label="Type Goods"
              append-icon="text_format"
              :counter="50"
            ></v-text-field>
...
export default {
  computed: {
    loading() {
      return this.$store.state.inventorysupplier.loading;
    },
    logo() {
      return this.$store.state.inventorysupplier.logo.data[0].fileName;
    },
    success() {
      return this.$store.state.inventorysupplier.success;
    },
    error() {
      return this.$store.state.inventorysupplier.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.inventorysupplier.items &&
        this.$store.state.inventorysupplier.items.rows
      ) {
        this.$store.state.inventorysupplier.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.inventorysupplier.items.total;
    },
    limit() {
      return this.$store.state.inventorysupplier.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.inventorysupplier.pagination;
    }
  },
  watch: {
    logo() {
      this.editedItem.logo = this.logo;
    },
    error() {
      this.showError = true;
    },
  ...
  created() {
    this.$store.dispatch("inventorysupplier/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
    ...
    async changeLogo() {
       // eslint-disable-next-line no-console
        ...
        await this.$store.dispatch("inventorysupplier/upload", this.files);
    },  
    async fakeItems() {
      for (var i = 0; i < 100; i++) {
        var payload = {
    ...
    async getItems() {
      ...
      await this.$store.dispatch("inventorysupplier/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("inventorysupplier/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("inventorysupplier/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
