      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">Commerce Product Infos</span>
      ...
      <v-card v-if="isadding">
        ...
        <v-card-text>
          <v-col cols="12" style="padding:0">
            ...
            <AutoComplete
              label="Info"
              service="commerce/info"
              v-bind:select="editedItem.info"
              v-on:change="changeInfo"
            />
...
export default {
  ...
  computed: {
    loading() {
      return this.$store.state.commerceproductinfo.loading;
    },
    success() {
      return this.$store.state.commerceproductinfo.success;
    },
    error() {
      return this.$store.state.commerceproductinfo.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.commerceproductinfo.items &&
        this.$store.state.commerceproductinfo.items.rows
      ) {
        this.$store.state.commerceproductinfo.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.commerceproductinfo.items.total;
    },
    limit() {
      return this.$store.state.commerceproductinfo.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.commerceproductinfo.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("commerceproductinfo/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
    ...
    changeProduct(value) {
     ...
     changeInfo(value) {
      ...
      this.editedItem.info = value._id;
    },
    ...
    async getItems() {
      ...
      await this.$store.dispatch("commerceproductinfo/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("commerceproductinfo/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("commerceproductinfo/updateItem", {
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("commerceproductinfo/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
