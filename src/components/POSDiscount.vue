      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">POS Discounts</span>
...
export default {
  computed: {
    loading() {
      return this.$store.state.posdiscount.loading;
    },
    success() {
      return this.$store.state.posdiscount.success;
    },
    error() {
      return this.$store.state.posdiscount.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.posdiscount.items &&
        this.$store.state.posdiscount.items.rows
      ) {
        this.$store.state.posdiscount.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.posdiscount.items.total;
    },
    limit() {
      return this.$store.state.posdiscount.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.posdiscount.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("posdiscount/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
    ...
    async getItems() {
      ...
      await this.$store.dispatch("posdiscount/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("posdiscount/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("posdiscount/updateItem", {
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("posdiscount/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
