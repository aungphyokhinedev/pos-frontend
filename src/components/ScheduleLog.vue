      ...
      <template style="width:20px;" v-slot:item.active="{ item }">
        ...
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">Schedule Logs</span>
...
export default {
  computed: {
    loading() {
      return this.$store.state.schedulelog.loading;
    },
    success() {
      return this.$store.state.schedulelog.success;
    },
    error() {
      return this.$store.state.schedulelog.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.schedulelog.items &&
        this.$store.state.schedulelog.items.rows
      ) {
        this.$store.state.schedulelog.items.rows.forEach(item => {
          var newitem = Object.assign({}, item);
          newitem.createdAt = moment(newitem.createdAt).format("D-MMM-YY");
           newitem.date = moment(newitem.date).format("D-MMM-YY hh:mm");
          result.push(newitem);
        });
      }
    ...
    total() {
      return this.$store.state.schedulelog.items.total;
    },
    limit() {
      return this.$store.state.schedulelog.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.schedulelog.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("schedulelog/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
    ...
    async getItems() {
      ...
      await this.$store.dispatch("schedulelog/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("schedulelog/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("schedulelog/updateItem", {
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("schedulelog/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
