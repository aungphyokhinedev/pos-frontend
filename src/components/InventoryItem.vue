      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">Invenotry Items</span>
      ...
      <v-card v-if="isadding">
        ...
        <v-card-text>
          <v-col cols="12" style="padding:0">
            ...
            <v-text-field
              dense
              v-model="editedItem.vendorProductCode"
            ...
            ></v-text-field>
             ...
             <AutoComplete
              label="SKU"
              service="inventory/sku"
              v-bind:select="editedItem.sku"
              v-on:change="changeSKU"
            />
            <v-text-field
              dense
            ...
            ></v-text-field>
             <v-text-field
              dense
              v-model="editedItem.unitInStock"
              label="Stock's Unit"
              append-icon="text_format"
              :counter="50"
            ></v-text-field>
             <v-text-field
              dense
              v-model="editedItem.unitInOrder"
              label="Order's Unit"
              append-icon="text_format"
              :counter="50"
            ></v-text-field>
            ...
            <v-text-field
              dense
              v-model="editedItem.reorderLevel"
              label="Reorder Level"
              type="number"
              append-icon="text_format"
              :counter="50"
...
export default {
  ...
  computed: {
    ...
    loading() {
      return this.$store.state.inventoryitem.loading;
    },
    success() {
      return this.$store.state.inventoryitem.success;
    },
    error() {
      return this.$store.state.inventoryitem.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.inventoryitem.items &&
        this.$store.state.inventoryitem.items.rows
      ) {
        this.$store.state.inventoryitem.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.inventoryitem.items.total;
    },
    limit() {
      return this.$store.state.inventoryitem.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.inventoryitem.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("inventoryitem/clearState");
    this.getItems();
  },
  methods: {
    changeBrand(value) {
    ...
    async getItems() {
      ...
      if (this.filterChange.query) {
        ...
        data.searchFields = "name,itemCode";
      }
      ...
      await this.$store.dispatch("inventoryitem/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("inventoryitem/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("inventoryitem/updateItem", {
            brand: this.editedItem.brand,
            category: this.editedItem.category,
            sku: this.editedItem.sku,
           
            vendorProductCode: this.editedItem.vendorProductCode,
            name: this.editedItem.name,
            itemCode: this.editedItem.itemCode,
            unitInStock: this.editedItem.unitInStock,
            unitInOrder: this.editedItem.unitInOrder,
            qtyPerUnit: this.editedItem.qtyPerUnit,
            size: this.editedItem.size,
            weight: this.editedItem.weight,
            color: this.editedItem.color,
            description: this.editedItem.description,
            reorderLevel: this.editedItem.reorderLevel,
            reorderQty: this.editedItem.reorderQty,
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("inventoryitem/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
