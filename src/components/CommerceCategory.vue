      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">Commerce Categories</span>
...
export default {
  computed: {
    loading() {
      return this.$store.state.commercecategory.loading;
    },
    success() {
      return this.$store.state.commercecategory.success;
    },
    error() {
      return this.$store.state.commercecategory.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.commercecategory.items &&
        this.$store.state.commercecategory.items.rows
      ) {
        this.$store.state.commercecategory.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.commercecategory.items.total;
    },
    limit() {
      return this.$store.state.commercecategory.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.commercecategory.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("commercecategory/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
    ...
    async getItems() {
      ...
      await this.$store.dispatch("commercecategory/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("commercecategory/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("commercecategory/updateItem", {
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("commercecategory/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
