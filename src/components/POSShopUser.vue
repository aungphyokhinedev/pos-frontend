      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">POS Shop and Users</span>
...
export default {
  ...
  computed: {
    loading() {
      return this.$store.state.posshopuser.loading;
    },
    success() {
      return this.$store.state.posshopuser.success;
    },
    error() {
      return this.$store.state.posshopuser.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.posshopuser.items &&
        this.$store.state.posshopuser.items.rows
      ) {
        this.$store.state.posshopuser.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.posshopuser.items.total;
    },
    limit() {
      return this.$store.state.posshopuser.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.posshopuser.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("posshopuser/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
    ...
    async getItems() {
      ...
      await this.$store.dispatch("posshopuser/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("posshopuser/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("posshopuser/updateItem", {
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("posshopuser/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
