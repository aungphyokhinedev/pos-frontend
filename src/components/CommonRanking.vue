      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">Rankings</span>
...
export default {
  ...
  computed: {
    loading() {
      return this.$store.state.commonranking.loading;
    },
    success() {
      return this.$store.state.commonranking.success;
    },
    error() {
      return this.$store.state.commonranking.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.commonranking.items &&
        this.$store.state.commonranking.items.rows
      ) {
        this.$store.state.commonranking.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.commonranking.items.total;
    },
    limit() {
      return this.$store.state.commonranking.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.commonranking.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("commonranking/clearState");
    this.getItems();
  },
  methods: {
    changeUser(value) {
    ...
    async getItems() {
      ...
      await this.$store.dispatch("commonranking/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("commonranking/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("commonranking/updateItem", {
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("commonranking/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
