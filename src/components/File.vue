  ...
  <v-content>
    <v-container fluid>
      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">Upload Files</span>
      ...
      <v-card v-if="isadding">
        <v-card-title>
          <span class="title1">{{isediting?'File Info':'Upload File'}}</span>
          <v-spacer />
        ...
        <v-card-text>
          <v-col v-if="isediting" cols="12" style="padding:0">
          <p>{{editedItem.name}}</p>
            <p>{{editedItem.size}}</p>
        ...
        <v-row v-if="!isediting" align="center" justify="center">
            <v-btn color="primary" dark @click="upload">Create Item</v-btn>
          </v-row>
...
export default {
  computed: {
    ...
    error() {
      return this.$store.state.file.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.file.items &&
        this.$store.state.file.items.rows
      ) {
        this.$store.state.file.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.file.items.total;
    },
    limit() {
      return this.$store.state.file.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.file.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("file/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
    ...
    async getItems() {
      ...
      await this.$store.dispatch("file/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("file/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async upload() {
        ...
        await this.$store.dispatch("file/upload", this.files)
        this.getItems();
        this.close();
    },
    close() {
