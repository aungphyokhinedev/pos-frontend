      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">POS Ratings</span>
...
export default {
  ...
  computed: {
    loading() {
      return this.$store.state.posrating.loading;
    },
    success() {
      return this.$store.state.posrating.success;
    },
    error() {
      return this.$store.state.posrating.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.posrating.items &&
        this.$store.state.posrating.items.rows
      ) {
        this.$store.state.posrating.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.posrating.items.total;
    },
    limit() {
      return this.$store.state.posrating.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.posrating.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("posrating/clearState");
    this.getItems();
  },
  methods: {
    changeUser(value) {
    ...
    async getItems() {
      ...
      await this.$store.dispatch("posrating/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("posrating/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("posrating/updateItem", {
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("posrating/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
