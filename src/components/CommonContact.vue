      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">Contacts</span>
...
export default {
  computed: {
    loading() {
      return this.$store.state.commoncontact.loading;
    },
    success() {
      return this.$store.state.commoncontact.success;
    },
    error() {
      return this.$store.state.commoncontact.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.commoncontact.items &&
        this.$store.state.commoncontact.items.rows
      ) {
        this.$store.state.commoncontact.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.commoncontact.items.total;
    },
    limit() {
      return this.$store.state.commoncontact.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.commoncontact.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("commoncontact/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
    ...
    async getItems() {
      ...
      if (this.filterChange.query) {
        ...
        data.searchFields = "name,phone,mobile,email";
      }
      ...
      await this.$store.dispatch("commoncontact/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("commoncontact/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("commoncontact/updateItem", {
            name: this.editedItem.name,
            phone: this.editedItem.phone,
            mobile: this.editedItem.mobile,
            email: this.editedItem.email,
            hotline: this.editedItem.hotline,
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("commoncontact/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
