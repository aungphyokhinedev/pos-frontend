      ...
      <template style="width:20px;" v-slot:item.active="{ item }">
        ...
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">Schedule Rules</span>
...
export default {
  computed: {
    loading() {
      return this.$store.state.schedulerule.loading;
    },
    success() {
      return this.$store.state.schedulerule.success;
    },
    error() {
      return this.$store.state.schedulerule.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.schedulerule.items &&
        this.$store.state.schedulerule.items.rows
      ) {
        this.$store.state.schedulerule.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.schedulerule.items.total;
    },
    limit() {
      return this.$store.state.schedulerule.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.schedulerule.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("schedulerule/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
    ...
    async getItems() {
      ...
      await this.$store.dispatch("schedulerule/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("schedulerule/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("schedulerule/updateItem", {
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("schedulerule/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
