      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">Commerce Discounts</span>
...
export default {
  computed: {
    loading() {
      return this.$store.state.commercediscount.loading;
    },
    success() {
      return this.$store.state.commercediscount.success;
    },
    error() {
      return this.$store.state.commercediscount.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.commercediscount.items &&
        this.$store.state.commercediscount.items.rows
      ) {
        this.$store.state.commercediscount.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.commercediscount.items.total;
    },
    limit() {
      return this.$store.state.commercediscount.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.commercediscount.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("commercediscount/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
    ...
    async getItems() {
      ...
      await this.$store.dispatch("commercediscount/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("commercediscount/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("commercediscount/updateItem", {
            name: this.editedItem.name,
            percentage: this.editedItem.percentage,
            flatRate: this.editedItem.flatRate,
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("commercediscount/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
