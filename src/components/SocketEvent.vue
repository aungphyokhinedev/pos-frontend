      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">Socket Events</span>
...
export default {
  computed: {
    loading() {
      return this.$store.state.socketevent.loading;
    },
    success() {
      return this.$store.state.socketevent.success;
    },
    error() {
      return this.$store.state.socketevent.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.socketevent.items &&
        this.$store.state.socketevent.items.rows
      ) {
        this.$store.state.socketevent.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.socketevent.items.total;
    },
    limit() {
      return this.$store.state.socketevent.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.socketevent.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("socketevent/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
    ...
    async getItems() {
      ...
      await this.$store.dispatch("socketevent/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("socketevent/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("socketevent/updateItem", {
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("socketevent/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
