      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">Inventory Brands</span>
        ...
        <template v-slot:item.shortName="{ item }">
          <a @click="edit(item)">{{item.shortName ? item.shortName: '-'}}</a>
        </template>
...
export default {
  computed: {
    loading() {
      return this.$store.state.inventorybrand.loading;
    },
    success() {
      return this.$store.state.inventorybrand.success;
    },
    error() {
      return this.$store.state.inventorybrand.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.inventorybrand.items &&
        this.$store.state.inventorybrand.items.rows
      ) {
        this.$store.state.inventorybrand.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.inventorybrand.items.total;
    },
    limit() {
      return this.$store.state.inventorybrand.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.inventorybrand.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("inventorybrand/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
    ...
    async getItems() {
      ...
      if (this.filterChange.query) {
        ...
        data.searchFields = "name,shortName";
      }
      ...
      await this.$store.dispatch("inventorybrand/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("inventorybrand/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("inventorybrand/updateItem", {
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("inventorybrand/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
