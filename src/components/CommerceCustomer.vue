      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">Commerce Customers</span>
          ...
          <v-col cols="12" style="padding:0">
            ...
            />
             <AutoComplete
              ...
              v-bind:select="editedItem.billingAddress"
              v-on:change="changeBillingAddress"
            />
             <AutoComplete
              label="Shipping Address"
              service="common/address"
              v-bind:select="editedItem.shippingAddress"
              v-on:change="changeShippingAddress"
            />
...
export default {
  ...
  computed: {
    loading() {
      return this.$store.state.commercecustomer.loading;
    },
    success() {
      return this.$store.state.commercecustomer.success;
    },
    error() {
      return this.$store.state.commercecustomer.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.commercecustomer.items &&
        this.$store.state.commercecustomer.items.rows
      ) {
        this.$store.state.commercecustomer.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.commercecustomer.items.total;
    },
    limit() {
      return this.$store.state.commercecustomer.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.commercecustomer.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("commercecustomer/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
    ...
    changeUser(value) {
      ...
      this.editedItem.user = value._id;
    },
     changeContact(value) {
     ...
     changeBillingAddress(value) {
      ...
      this.editedItem.billingAddress = value._id;
    },
    changeShippingAddress(value) {
     
      this.editedItem.shippingAddress = value._id;
    },
    next() {
      if (!this.isadding) {
        this.add();
    ...
    async getItems() {
      var data = {
        ...
        populate: [ "user","contact","billingAddress","shippingAddress"]
      };
      // eslint-disable-next-line no-console
      ...
      await this.$store.dispatch("commercecustomer/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("commercecustomer/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("commercecustomer/updateItem", {
            name: this.editedItem.name,
            user: this.editedItem.user,
            contact: this.editedItem.contact,
            billingAddress: this.editedItem.billingAddress,
            shippingAddress: this.editedItem.shippingAddress,
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("commercecustomer/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
