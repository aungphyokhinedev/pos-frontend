      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">POS Shops</span>
          ...
          <v-col cols="12" style="padding:0">
           ...
           <v-text-field
            ...
            ></v-text-field>    
              ...
              <v-checkbox dense v-model="editedItem.locked" label="Locked" required></v-checkbox>
            </v-row>
          </v-col>
...
export default {
  ...
  computed: {
    loading() {
      return this.$store.state.posshop.loading;
    },
    success() {
      return this.$store.state.posshop.success;
    },
    error() {
      return this.$store.state.posshop.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.posshop.items &&
        this.$store.state.posshop.items.rows
      ) {
        this.$store.state.posshop.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.posshop.items.total;
    },
    limit() {
      return this.$store.state.posshop.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.posshop.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("posshop/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
    ...
    async getItems() {
      ...
      await this.$store.dispatch("posshop/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("posshop/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("posshop/updateItem", {
            ...
            coverage: this.editedItem.coverage,
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("posshop/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
