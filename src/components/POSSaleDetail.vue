      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">POS Sale Details</span>
          ...
          <v-col cols="12" style="padding:0">
            ...
            ></v-text-field>
             <AutoComplete
              label="Sale"
              service="pos/sale"
              v-bind:select="editedItem.sale"
              v-on:change="changeSale"
            />
            <AutoComplete
              label="Item"
              service="pos/item"
...
export default {
  ...
  computed: {
    loading() {
      return this.$store.state.possaledetail.loading;
    },
    success() {
      return this.$store.state.possaledetail.success;
    },
    error() {
      return this.$store.state.possaledetail.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.possaledetail.items &&
        this.$store.state.possaledetail.items.rows
      ) {
        this.$store.state.possaledetail.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.possaledetail.items.total;
    },
    limit() {
      return this.$store.state.possaledetail.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.possaledetail.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("possaledetail/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
  ...
  changeSale(value) {
        this.editedItem.sale = value._id;
    },
    next() {
      if (!this.isadding) {
        this.add();
    ...
    async getItems() {
      ...
      if (this.filterChange.query) {
        ...
        data.searchFields = "name,invoiceNumber";
      }
      ...
      await this.$store.dispatch("possaledetail/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("possaledetail/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("possaledetail/updateItem", {
            name: this.editedItem.name,
            invoiceNumber: this.editedItem.invoiceNumber,
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("possaledetail/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
