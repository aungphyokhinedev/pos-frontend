      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">Access Rules</span>
        ...
        <v-card-text>
          <v-col cols="12" style="padding:0">
            ...
            <AutoComplete
              label="User Group"
              service="usergroup"
              v-bind:select="editedItem.userGroup"
              v-on:change="changeUserGroup"
            />
            <AutoComplete
              label="Application"
              service="application"
              v-bind:select="editedItem.application"
              v-on:change="changeApplication"
            />
            <AutoComplete
              label="Service"
              service="service"
              v-bind:select="editedItem.service"
...
export default {
  ...
  computed: {
    loading() {
      return this.$store.state.accessrule.loading;
    },
    success() {
      return this.$store.state.accessrule.success;
    },
    error() {
      return this.$store.state.accessrule.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.accessrule.items &&
        this.$store.state.accessrule.items.rows
      ) {
        this.$store.state.accessrule.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.accessrule.items.total;
    },
    limit() {
      return this.$store.state.accessrule.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.accessrule.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("accessrule/clearState");
    this.getItems();
  },
  methods: {
    next() {
    ...
    changeUserGroup(value) {
      this.editedItem.userGroup = value;
    },
    changeApplication(value) {
      this.editedItem.application = value;
    },
    changeService(value) {
      this.editedItem.service = value;
    },
    changeRule(value) {
    ...
    async fakeItems() {
      for (var i = 0; i < 100; i++) {
        ...
        await this.$store.dispatch("accessrule/createItem", payload);
      }
    },
    add() {
      this.editedItem = {};
    ...
    edit(item) {
      ...
      console.log(item);
      item.application = item.application ? item.application : { name: "" };
    ...
    async getItems() {
      var data = {
        ...
        populate: ["service", "userGroup", "rule", "application"]
      };
      // eslint-disable-next-line no-console
      ...
      await this.$store.dispatch("accessrule/getItems", data);
    },
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("accessrule/updateItem", {
            name: this.editedItem.name,
            _id: this.editedItem._id,
            userGroup: this.editedItem.userGroup._id,
            application: this.editedItem.application._id,
            service: this.editedItem.service._id,
            rule: this.editedItem.rule._id
          })
        ) {
          this.getItems();
          this.close();
        ...
        console.log("add new");
        if (
          await this.$store.dispatch("accessrule/createItem", {
            name: this.editedItem.name,
            userGroup: this.editedItem.userGroup._id,
            application: this.editedItem.application._id,
            service: this.editedItem.service._id,
