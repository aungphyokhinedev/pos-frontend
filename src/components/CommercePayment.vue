      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">Commerce Payments</span>
...
<template style="width:20px;" v-slot:item.done="{ item }">
          <v-chip outlined style="margin: 2px 0" small v-if="item.done">
            <v-icon small left color="green">mdi-check</v-icon>Done
          </v-chip>
        </template>
        ...
        <v-card-text>
          <v-col cols="12" style="padding:0">
            ...
            <v-row style="padding:10px" v-if="isediting">
              <v-checkbox class="mr-3" dense v-model="editedItem.done" label="Done" required></v-checkbox>
...
export default {
  ...
  computed: {
    loading() {
      return this.$store.state.commercepayment.loading;
    },
    success() {
      return this.$store.state.commercepayment.success;
    },
    error() {
      return this.$store.state.commercepayment.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.commercepayment.items &&
        this.$store.state.commercepayment.items.rows
      ) {
        this.$store.state.commercepayment.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.commercepayment.items.total;
    },
    limit() {
      return this.$store.state.commercepayment.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.commercepayment.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("commercepayment/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
    ...
    changeType(value) {
      ...
      this.editedItem.type = value._id;
    },
    ...
    async getItems() {
      ...
      await this.$store.dispatch("commercepayment/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("commercepayment/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("commercepayment/updateItem", {
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("commercepayment/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
