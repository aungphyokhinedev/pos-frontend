      ...
      >
        <template v-slot:top>
          <v-toolbar dark color="primary" flat>
            ...
            <span class="title mr-2">Account Types</span>
        ...
        <template v-slot:item.typeCode="{ item }">
          <a @click="edit(item)">{{item.typeCode ? item.typeCode: '-'}}</a>
        </template>
...
export default {
  computed: {
    loading() {
      return this.$store.state.accounttype.loading;
    },
    success() {
      return this.$store.state.accounttype.success;
    },
    error() {
      return this.$store.state.accounttype.error;
    },
    searching() {
    ...
    items() {
      var result = [];
      if (
        this.$store.state.accounttype.items &&
        this.$store.state.accounttype.items.rows
      ) {
        this.$store.state.accounttype.items.rows.forEach(item => {
    ...
    total() {
      return this.$store.state.accounttype.items.total;
    },
    limit() {
      return this.$store.state.accounttype.items.limit;
    },
    pageChange() {
      return (
    ...
    pagination() {
      return this.$store.state.accounttype.pagination;
    }
  },
  watch: {
    error() {
  ...
  created() {
    this.$store.dispatch("accounttype/clearState");
    this.getItems();
  },
  methods: {
    async clear() {
    ...
    async getItems() {
      ...
      await this.$store.dispatch("accounttype/getItems", data);
    },

    async deleteUser() {
      if (confirm("Are you sure you want to delete this item?")) {
        if (
          await this.$store.dispatch("accounttype/deleteItem", this.editedItem._id)
        ) {
          this.getItems();
          this.close();
        }
    ...
    async save() {
      if (this.isediting) {
        ...
        console.log("editing");
        if (
          await this.$store.dispatch("accounttype/updateItem", {
            name: this.editedItem.name,
            typeCode: this.editedItem.typeCode,
        ...
        // eslint-disable-next-line no-console
        console.log("add new");
        if (await this.$store.dispatch("accounttype/createItem", this.editedItem)) {
          this.getItems();
          this.close();
        }
      }
